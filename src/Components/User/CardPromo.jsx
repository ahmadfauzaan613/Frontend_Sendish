...
function CardPromo(props) {
  const { key, image, alt, namaProduk, diskon, hargaLama, harga } = props
  return (
    <React.Fragment>
      <div className="px-3" key={key}>
        <div className="bg-white w-full rounded-md h-full py-2 px-3 shadow-lg">
          ...
          <div className="flex items-center space-x-3 py-3">
            <p className="text-sm font-bold bg-[#ff2020] w-fit p-1 text-white rounded-md">{diskon}</p>
            <p className="text-sm  text-[#9c9c9c] line-through">{hargaLama}</p>
          </div>
